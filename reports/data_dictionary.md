...
## Event Labels (`data/events/*.csv`)

| Column | Meaning | Source | Notes |
|--------|---------|--------|-------|
| `event_idx` | Event sequence number | Auto-increment | |
| `window_end_ts` | Event timestamp (epoch seconds) | Detection window end | |
| `window_end_utc` | Event timestamp (UTC) | Derived | |
| `window_start_ts` | Window start (epoch seconds) | Detection window start | |
| `window_start_utc` | Window start (UTC) | Derived | |
| `delta_cvd_30s` | CVD change in 30s window | Sum of net_delta | |
| `return_30s` | Price return in 30s window | (close − open) / open | |
| `trade_intensity_30s` | Trade count in 30s window | Sum of total_trades | |
| `price_start` | Price at window start | price_close at start | |
| `price_end` | Price at window end | price_close at end | |
| `cvd_at_trigger` | CVD value at trigger | cvd_cumulative | |
| `p5_cvd_threshold` | 5th percentile CVD threshold | Rolling prior-only | |
| `p75_intensity_threshold` | 75th percentile intensity | Rolling prior-only | |

## Winsorization Bounds (`data/processed/winsor_bounds_*.json`)
